...
public class ResponseActorRouter extends AbstractActor {
    ...
    public static Props props() {
        return Props.create(ResponseActorRouter.class, ResponseActorRouter::new);
    }
    ...
    private void notifyResponse(CommandResponse commandResponse) {
        ...
        if (actorRef != null) {
            log.info("Command {} response can now be routed to the sender", commandResponse.getId());
            actorRef.tell(commandResponse, self());
        } else {
            log.warning("Actor Receiver not found for command {}", commandResponse.getId());
        }
    }

    private void registerActor(ResponseWanted responseWanted) {
        log.info("Setting Actor for {} request response ", responseWanted.getCommandId());
